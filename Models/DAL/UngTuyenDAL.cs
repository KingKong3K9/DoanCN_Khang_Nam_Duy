...
using DoanCN_Khang_Nam_Duy.Models.ViewModels.HoSoXinViec;
using DoanCN_Khang_Nam_Duy.Models.ViewModels.UngTuyen;

namespace DoanCN_Khang_Nam_Duy.Models.DAL
{
    public class UngTuyenDAL
    {
        ...
        public ResultPaging<UngTuyenVm> GetListByNTD(bool status, int month, GetListPaging paging, int idNguoiDung)
        {
            var model = from a in dbContext.tbl_UngTuyen
                        ...
                        join c in dbContext.tbl_HoSoXinViec
                        on a.PK_iMaUngVien equals c.FK_iMaUngVien into hoSoJoin // LEFT JOIN với tbl_HoSoXinViec
                        from c in hoSoJoin.DefaultIfEmpty()
                        where b.FK_iMaNTD == idNguoiDung
                        select new
                        {
                            UngTuyen = a,
                            TinTuyenDung = b,
                            HoSoXinViec = c // Gán dữ liệu từ tbl_HoSoXinViec vào kết quả
                        };
            ...
            var listItem = model.ToList().Select(x => new UngTuyenVm()
            {
                ...
                TrangThai = x.UngTuyen.sTrangThai,
                TrangThaiTimViec = x.HoSoXinViec.sTrangThai,
            }).ToList();
